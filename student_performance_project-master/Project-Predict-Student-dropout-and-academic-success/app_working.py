...
def train_new_model():
    ...
    try:
        ...
        # Create sample features based on the expected columns (fixed typos)
        column_names = [
        ...
        # Use simplified logic to avoid column name issues
        tuition_fees_not_up_to_date = df["Tuition fees up to date"] == 0
        no_scholarship = df["Scholarship holder"] == 0
        low_approved_units = df["Curricular units 1st sem (approved)"] < 5
        high_age = df["Age at enrollment"] > 25
        ...
        # Save the new model
        joblib.dump(model_pipeline, 'my_trained_model_working.joblib')
...
def load_model():
    ...
    try:
        ...
        if os.path.exists('my_trained_model_working.joblib'):
            model_pipeline = joblib.load('my_trained_model_working.joblib')
...
def predict_from_csv(filepath):
    ...
    try:
        ...
        # Add column names to the dataframe (fixed typos)
        column_names = [
            "Marital status", "Application mode", "Application order", "Course", 
            "Daytime/evening attendance", "Previous qualification", "Nationality", 
            "Mother's qualification", "Father's qualification", "Mother's occupation", 
...
def predict_manual():
    ...
    try:
        if request.method == 'POST':
            ...
            # Create DataFrame with all expected columns (fixed typos)
            column_names = [
                "Marital status", "Application mode", "Application order", "Course", 
                "Daytime/evening attendance", "Previous qualification", "Nationality", 
                "Mother's qualification", "Father's qualification", "Mother's occupation", 
